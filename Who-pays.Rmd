...
Looking at [census data page](https://api.census.gov/data/timeseries/intltrade/imports/hs/variables.html), I decide to select "GEN_VAL_MO" as value, "CON_QY1_MO" as quantity1, and "CON_QY2_MO" as quantity2, though I selected "GEN_CIF_MO" as value in [the original analysis](README.md).
...
sum2hs8 <- function(df) {
    ...
    df$CON_QY2_MO <- as.numeric(df$CON_QY2_MO)
    ...
    df %>% 
      rename(
        hs10 = I_COMMODITY,
        value = GEN_VAL_MO,
        quantity1 = CON_QY1_MO,
        unit1 = UNIT_QY1,
        quantity2 = CON_QY2_MO,
        unit2 = UNIT_QY2
      ) %>% 
      mutate(
        hs8 = str_sub(hs10, end = -3L) # cut codes from 10 to 8 digits
      ) %>% 
      select(time, hs10, hs8, value, quantity1, unit1, quantity2, unit2) %>% 
      arrange(time, hs10)
  }
...
import_from_country <- function(country, year) {
  df <- censusapi::getCensus(
    ...
    vars = c("I_COMMODITY", "GEN_VAL_MO", "CON_QY1_MO", "UNIT_QY1", "CON_QY2_MO", "UNIT_QY2"),
    time = year,
    CTY_CODE = country,
    COMM_LVL = "HS10"
    )
...
If unit is inconsistent over years, my method of calculation of price index is meaningless. Let us check 'unit1' column, which is supposed to be 3-character import unit of quantity.
