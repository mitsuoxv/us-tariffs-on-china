...
# goods of whch imports from china declines in 2019 4Q by more than 200 million
# dollars
hs_china_loss <- import_q %>% 
  filter(country == "CHINA") %>% 
  tq_diff(n = 4) %>% 
  as_tsibble() %>% 
  filter(quarter == "2019-10-01", value < -200) %>% 
  `[[`("hs")

# and imports from china in 2019 4Q still more than 500 million dollars
hs_china_loss2 <- import_q %>% 
  filter(hs %in% hs_china_loss) %>% 
  filter(quarter == "2019-10-01", value > 500) %>% 
  `[[`("hs")

```

I first choose HS codes of which imports from China declined by more than 200 million dollars in 2019 4Q. Next I choose HS codes of which imports from China are still more than 500 million dollars in 2019 4Q.
