...
# goods of whch imports from china declines by more than 200 million
# dollars from 2018 Q1
hs_china_loss <- import_q_diff %>% 
  filter(country == "CHINA") %>% 
  filter(quarter == yearquarter("2020-10-01"), value < -200) %>% 
  pull("hs")

# and imports from china still more than 500 million dollars
hs_china_loss2 <- import_q %>% 
  filter(hs %in% hs_china_loss) %>% 
  filter(quarter == yearquarter("2020-10-01"), value > 500) %>% 
  pull("hs") %>% 
  unique()
...
Most notable case is that Vietnam replaces China as a main export base of 8541 (Diodes, transistors) and 8542 (Integrated circuits).

Vietnam is also advancing in 9401 (Seats) and 9403 (Other furniture).

South Korea supplants China in 8415 (Air conditioning machines) and 8523 (Recording medium).
