...
I hear GPU price jump in February 2021 is partially due to tariff exclusion expiration. However, I don't know how to reflect expiration. So, I assume wrongly that granted exclusion will never expire.
...
```{r save}
# remove double
df_list_34b <- df_list_34b |> 
  distinct(trf_34b, hs8)

df_list_16b <- df_list_16b |> 
  distinct(trf_16b, hs8)

df_list_200b <- df_list_200b |> 
  distinct(trf_200b, hs8)

df_list_300b_a <- df_list_300b_a |> 
  distinct(trf_300b_a, hs8)

df_list_300b_c <- df_list_300b_c |> 
  distinct(trf_300b_c, hs8)
